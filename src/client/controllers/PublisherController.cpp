...
void PublisherController::createBook()
{
    ...
    //   Note: basePrice is a JSON number, so we must check it via isDouble()
    //   rather than toString().isEmpty() — QJsonValue::toString() returns an
    //   empty string for numeric values, which would falsely flag basePrice
    //   as missing even when it's set.
    if (m_bookData["title"].toString().trimmed().isEmpty()) {
        LOG_WARNING("Create book failed: missing required field: title");
        emit errorOccurred("Missing required field: title");
        return;
    }
    if (m_bookData["authorName"].toString().trimmed().isEmpty()) {
        LOG_WARNING("Create book failed: missing required field: authorName");
        emit errorOccurred("Missing required field: authorName");
        return;
    }
    if (!m_bookData.contains("basePrice") || !m_bookData["basePrice"].isDouble()
        || m_bookData["basePrice"].toDouble() <= 0.0) {
        LOG_WARNING("Create book failed: missing or invalid required field: basePrice");
        emit errorOccurred("Missing or invalid required field: basePrice");
        return;
    }
...
void PublisherController::handlePublishBookResponse(const common::Message& response)
{
    ...
    // Refresh the cached book list from the server so subscribers see the
    // newly published book. Previously this only emitted bookListChanged()
    // without updating m_booksData, so QML subscribers re-queried and got
    // the stale list (the new book was missing).
    loadMyBooks();
    emit bookListChanged();
...
void PublisherController::handleUpdateBookResponse(const common::Message& response)
{
    ...
    // Refresh the cached book list so subscribers see the updated fields.
    loadMyBooks();
    emit bookListChanged();
...
void PublisherController::handleDeactivateBookResponse(const common::Message& response)
{
    ...
    // Refresh the cached book list so subscribers see the deactivation.
    loadMyBooks();
    emit bookListChanged();
...
void PublisherController::handleActivateBookResponse(const common::Message& response)
{
    ...
    // Refresh the cached book list so subscribers see the activation.
    loadMyBooks();
    emit bookListChanged();
...
void PublisherController::handleApplyTimedDiscountResponse(const common::Message& response)
{
    ...
    // Refresh the cached book list so subscribers see the discounted prices.
    loadMyBooks();
    emit bookListChanged();
