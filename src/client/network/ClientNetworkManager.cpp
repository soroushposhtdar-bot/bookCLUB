// src/client/network/ClientNetworkManager.cpp
#include "src/client/network/ClientNetworkManager.h"
#include "common/Utils/Logger.h"
#include "common/Network/Protocol.h"

#include <QHostAddress>
#include <QJsonDocument>
#include <QJsonObject>

namespace bookclub::client {

ClientNetworkManager& ClientNetworkManager::instance() {
    static ClientNetworkManager instance;
    return instance;
}

ClientNetworkManager::ClientNetworkManager(QObject* parent)
    : QObject(parent)
{
    m_socket = new QTcpSocket(this);
    m_reconnectTimer = new QTimer(this);
    m_reconnectTimer->setSingleShot(true);
    m_reconnectTimer->setInterval(3000);

    connect(m_socket, &QTcpSocket::connected, this, &ClientNetworkManager::onConnected);
    connect(m_socket, &QTcpSocket::disconnected, this, &ClientNetworkManager::onDisconnected);
    connect(m_socket, &QTcpSocket::readyRead, this, &ClientNetworkManager::onReadyRead);
    connect(m_socket, &QAbstractSocket::errorOccurred,
            this, &ClientNetworkManager::onError);
    connect(m_reconnectTimer, &QTimer::timeout, this, &ClientNetworkManager::onReconnectTimeout);

    LOG_INFO("ClientNetworkManager initialized");
}

ClientNetworkManager::~ClientNetworkManager() {
    disconnectFromServer();
}

bool ClientNetworkManager::connectToServer(const QString& host, quint16 port) {
    if (m_connected) {
        LOG_WARNING("Already connected to server");
        return true;
    }

    m_host = host;
    m_port = port;

    LOG_INFO("Connecting to server: " + host + ":" + QString::number(port));
    // Initiate async connect — the connected() signal fires on success.
    // We don't block here; the caller (usually NetworkService::sendRequest)
    // either gets a fast-fail from the socket error or a successful
    // connection within milliseconds on localhost.
    m_socket->connectToHost(host, port);

    // Short wait (500ms) so synchronous callers can detect immediate
    // failures (e.g., server not running). The full 5s Qt timeout is
    // handled by the socket internally; if it fires, onError() will
    // emit errorOccurred() and schedule a reconnect.
    if (!m_socket->waitForConnected(500)) {
        // Don't log this as an error — the async connect may still
        // succeed shortly. The caller will see isConnected() == false
        // and can retry.
        LOG_DEBUG("Async connect pending for: " + host + ":" + QString::number(port));
        return false;
    }

    return true;
}

void ClientNetworkManager::disconnectFromServer() {
    if (!m_connected && !m_socket) return;

    LOG_INFO("Disconnecting from server");
    if (m_socket) {
        m_socket->disconnectFromHost();
        if (m_socket->state() != QAbstractSocket::UnconnectedState) {
            m_socket->waitForDisconnected(3000);
        }
    }
    m_connected = false;
    m_reconnectTimer->stop();
}

bool ClientNetworkManager::isConnected() const {
    return m_connected && m_socket && m_socket->state() == QAbstractSocket::ConnectedState;
}

void ClientNetworkManager::sendMessage(const common::Message& message) {
    if (!isConnected()) {
        LOG_ERROR("Cannot send message: not connected to server");
        emit errorOccurred("Not connected to server");
        return;
    }

    QByteArray packet = common::PacketParser::pack(message);
    qint64 written = m_socket->write(packet);
    if (written != packet.size()) {
        LOG_ERROR("Failed to send complete message. Written: " + QString::number(written) +
                  ", Expected: " + QString::number(packet.size()));
        emit errorOccurred("Failed to send message");
        return;
    }
    m_socket->flush();

    LOG_DEBUG("Message sent: " + common::commandToString(message.command())
              + " (requestId=" + message.requestId() + ")");
}

void ClientNetworkManager::sendRequest(common::Command command, const QJsonObject& payload) {
    common::Message msg(command, payload);
    sendMessage(msg);
}

void ClientNetworkManager::registerRequestHandler(common::Command command, MessageHandler handler) {
    QMutexLocker locker(&m_mutex);
    m_requestHandlers[command] = handler;
    LOG_DEBUG("Registered per-Command handler for: " + common::commandToString(command));
}

void ClientNetworkManager::unregisterRequestHandler(common::Command command) {
    QMutexLocker locker(&m_mutex);
    m_requestHandlers.remove(command);
}

void ClientNetworkManager::registerResponseHandler(const QString& requestId, MessageHandler handler) {
    if (requestId.isEmpty()) return;
    QMutexLocker locker(&m_mutex);
    m_responseHandlers[requestId] = handler;
}

void ClientNetworkManager::unregisterResponseHandler(const QString& requestId) {
    if (requestId.isEmpty()) return;
    QMutexLocker locker(&m_mutex);
    m_responseHandlers.remove(requestId);
}

void ClientNetworkManager::registerEventHandler(common::Command eventCommand, MessageHandler handler) {
    QMutexLocker locker(&m_mutex);
    m_eventHandlers[eventCommand] = handler;
    LOG_DEBUG("Registered event handler for: " + common::commandToString(eventCommand));
}

void ClientNetworkManager::unregisterEventHandler(common::Command eventCommand) {
    QMutexLocker locker(&m_mutex);
    m_eventHandlers.remove(eventCommand);
}

// --- Private Slots ---

void ClientNetworkManager::onConnected() {
    m_connected = true;
    m_reconnectTimer->stop();
    LOG_INFO("Connected to server: " + m_host + ":" + QString::number(m_port));
    emit connected();
}

void ClientNetworkManager::onDisconnected() {
    m_connected = false;
    LOG_INFO("Disconnected from server");
    emit disconnected();

    if (!m_host.isEmpty() && m_port > 0) {
        LOG_INFO("Scheduling reconnect in 3 seconds...");
        m_reconnectTimer->start();
    }
}

void ClientNetworkManager::onReadyRead() {
    if (!m_socket) return;

    // Read whatever is available and feed it to the packet parser.
    // Use readAll() — Qt buffers internally and emits readyRead per chunk;
    // partial packets are handled by PacketParser's length-prefix framing.
    QByteArray data = m_socket->readAll();
    m_packetParser.feed(data);
    processPendingPackets();
}

void ClientNetworkManager::processPendingPackets() {
    while (m_packetParser.hasNextPacket()) {
        common::Message message = m_packetParser.nextPacket();
        if (!message.isValid()) {
            LOG_WARNING("Received invalid packet");
            continue;
        }

        LOG_DEBUG("Received message: " + common::commandToString(message.command())
                  + " (requestId=" + message.requestId() + ")");
        emit messageReceived(message);
        routeResponse(message);
    }
}

void ClientNetworkManager::routeResponse(const common::Message& response) {
    const common::Command cmd = response.command();
    const QString reqId = response.requestId();

    // Server-pushed events (Command >= 1000): only fire the long-lived
    // event handler. These don't have a matching requestId.
    if (common::isEventCommand(cmd)) {
        QMutexLocker locker(&m_mutex);
        auto it = m_eventHandlers.find(cmd);
        if (it != m_eventHandlers.end() && it.value()) {
            locker.unlock();
            it.value()(response);
        }
        return;
    }

    // Per-requestId handler (one-shot) — preferred path for request/response.
    if (!reqId.isEmpty()) {
        QMutexLocker locker(&m_mutex);
        auto it = m_responseHandlers.find(reqId);
        if (it != m_responseHandlers.end() && it.value()) {
            MessageHandler handler = it.value();
            m_responseHandlers.erase(it);
            locker.unlock();
            handler(response);
            emit requestCompleted(cmd, response);
            return;
        }
    }

    // Fallback: per-Command handler (legacy + services that don't use reqId).
    QMutexLocker locker(&m_mutex);
    auto it = m_requestHandlers.find(cmd);
    if (it != m_requestHandlers.end() && it.value()) {
        locker.unlock();
        it.value()(response);
        emit requestCompleted(cmd, response);
    } else {
        LOG_DEBUG("No handler registered for response: " + common::commandToString(cmd));
    }
}

void ClientNetworkManager::onError(QAbstractSocket::SocketError error) {
    QString errorMsg = m_socket->errorString();
    LOG_ERROR("Socket error: " + errorMsg + " (Code: " + QString::number(error) + ")");
    emit errorOccurred(errorMsg);

    if (m_connected) {
        m_connected = false;
        emit disconnected();
        if (!m_host.isEmpty() && m_port > 0) {
            LOG_INFO("Scheduling reconnect in 3 seconds...");
            m_reconnectTimer->start();
        }
    }
}

void ClientNetworkManager::onReconnectTimeout() {
    LOG_INFO("Attempting to reconnect to " + m_host + ":" + QString::number(m_port));
    // Only reconnect if the socket is in the UnconnectedState. If it's
    // still Connecting or already Connected, calling connectToHost again
    // would cause QAbstractSocket::UnsupportedSocketOperationError.
    if (m_socket->state() != QAbstractSocket::UnconnectedState) {
        LOG_DEBUG("Skipping reconnect — socket state is "
                  + QString::number(m_socket->state()));
        // Schedule another retry in case it eventually disconnects.
        m_reconnectTimer->start();
        return;
    }
    m_socket->connectToHost(m_host, m_port);
    // Don't block — onConnected() will fire if it succeeds, otherwise
    // onError() will schedule another retry.
}

} // namespace bookclub::client
