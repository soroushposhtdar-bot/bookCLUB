...
int ServerApplication::run(int argc, char* argv[])
{
    ...
    if (!parseArguments(m_app->arguments())) {
        QFile errFile(QCoreApplication::applicationDirPath() + "/startup_error.txt");
        if (errFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QTextStream ts(&errFile);
            ts << "[BookClub Server] parseArguments() failed. Check command-line options.\n";
        }
        return 1;
    }
    ...
    if (!m_server->initialize(m_dbPath, m_port)) {
        ...
        if (errFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QTextStream ts(&errFile);
            ts << "[BookClub Server] initialize() failed.\n";
            ts << "  Database path: " << m_dbPath << "\n";
            ts << "  Port: " << m_port << "\n";
            ts << "  Check logs/server.log for details.\n";
        }
        return 1;
    }
