...
void ConnectionManager::onNewConnection() {
    while (m_server->hasPendingConnections()) {
        ...
        // Pass the already-created socket to ClientConnection. It takes ownership
        // and reparents the socket to itself.
        auto* connection = new ClientConnection(socket, this, m_router, this);
...
void ConnectionManager::registerUser(const QString& userId, const QString& clientId) {
    if (userId.isEmpty() || clientId.isEmpty()) return;
    QMutexLocker locker(&m_mutex);
    m_userToClientMap[userId] = clientId;
}

void ConnectionManager::unregisterUser(const QString& userId) {
    if (userId.isEmpty()) return;
    QMutexLocker locker(&m_mutex);
    m_userToClientMap.remove(userId);
}
