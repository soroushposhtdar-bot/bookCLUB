...
public:
    ...
    QVector<Book*> searchBooks(const QString& keyword) const override {
        // Multi-field search: title + author + publisher name. The spec
        // (§2-2) requires searching by all three fields simultaneously.
        // We run three searches and deduplicate by book ID.
        QVector<Book*> results;
        QSet<QString> seen;

        auto addUnique = [&](QVector<Book*> books) {
            for (Book* b : books) {
                if (!seen.contains(b->id())) {
                    seen.insert(b->id());
                    results.append(b);
                } else {
                    delete b;  // duplicate — free it
                }
            }
        };

        addUnique(m_bookRepo->searchByTitle(keyword));
        addUnique(m_bookRepo->searchByAuthor(keyword));
        addUnique(m_bookRepo->searchByPublisherName(keyword));
        return results;
    }
