// =============================================================================
//  PublisherBookDetailDrawer.qml
// =============================================================================
//  Slide-in drawer for the publisher role showing full details of a single
//  book in the catalog. Replaces the old "edit toast" placeholder with a real
//  detail view: cover, title/author/publisher, price/discount, rating +
//  rating-count, sales count, status badge, description, and a reviews list.
//
...
//  `bookDetail(bookId)` → QVariantMap with all book fields + a `reviews`
//  QVariantList. The drawer reloads whenever booksChanged fires so changes
//  from the editor or status toggle reflect immediately.
//
//  Usage:
...
//        onEditRequested: _catalogPage._bookEditor.openEdit(book, bookId)
//    }
//    _bookDrawer.openForBook("b001")
// =============================================================================
import QtQuick 2.15
...
Item {
    ...
    // ----- PublisherViewModel (injected by parent) -----
    ...
    // Hide timer — interval matches _slideOut.duration (Theme.motion.durationBase)
    // so the drawer is hidden exactly when the slide-out animation finishes.
    // Previously this was hardcoded to 260ms, which would drift if
    // durationBase ever changed.
    Timer {
        id: _hideTimer
        ...
        onTriggered: {
            ...
            // pending bookId). Previously this signal was declared but never
            // emitted, so subscribers never fired.
            drawer.closed()
        }
    }

    // Reload whenever the VM signals booksChanged (covers edits + status toggles).
    Connections {
    ...
    Rectangle {
        ...
        Column {
            ...
            ScrollView {
                width: parent.width
                height: parent.height - 72 - 88   // header + footer action bar
                clip: true
                    ...
                    spacing: Theme.space.lg
                        ...
                        Row {
                            ...
                            Repeater {
                                model: [
                                    ...
                                    { label: "Sales",     value: (drawer._detail.totalSales || 0).toLocaleString(Qt.locale(), "f", 0), color: Theme.color.success },
                                    { label: "Rating",    value: (drawer._detail.averageRating || 0).toFixed(1), color: Theme.color.warning },
                                    { label: "Reviews",   value: (drawer._detail.ratingCount || 0).toLocaleString(Qt.locale(), "f", 0), color: Theme.color.info }
                                ]
                                Rectangle {
                        ...
                        Card {
                            ...
                            Column {
                                ...
                                SectionHeader { width: parent.width; title: "Genres" }
                                Row {
                                    width: parent.width
                                    spacing: Theme.space.xs
                                    Repeater {
                                        ...
                                        Rectangle {
                                            width: _genreLbl.implicitWidth + 16
                                            height: 22
                                            radius: 11
                                    ...
                                    // Empty state for no-genre books
                                    Text {
                        ...
                        Card {
                            ...
                            Column {
                                ...
                                ListView {
                                    width: parent.width
                                    height: Math.min(360, Math.max(0, (drawer._detail.reviews || []).length) * 100)
                                    clip: true
