...
Item {
    ...
    Rectangle {
        ...
        // ----- Hero Panel (left) -----
        // clip: true is CRITICAL — without it, the decorative Canvas elements
        // below would overflow the panel and bleed into the form area on the
        // right (Issue 1 in the bug-fix batch).
        Item {
            id: _hero
            ...
            // Organic curve decoration (top-right) — clipped to panel bounds
            Canvas {
                anchors.fill: parent
                onPaint: {
                    ...
                    ctx.beginPath()
                    ctx.moveTo(w * 0.55, 0)
                    ctx.bezierCurveTo(w * 0.85, h * 0.10, w * 0.95, h * 0.45, w * 1.00, h * 0.78)
                    ctx.bezierCurveTo(w * 1.00, h * 0.92, w * 0.92, h, w * 0.78, h)
                    ctx.lineTo(0, h)
                    ctx.lineTo(0, 0)
                    ctx.closePath()
                    ctx.fillStyle = Qt.rgba(255/255, 255/255, 255/255, 0.55)
                    ctx.fill()
                }
            }

            // Decorative book spines (faint, bottom-right corner)
            // All anchors + sizes are constrained INSIDE the panel so the
            // illustration never overflows the form area below.
            Canvas {
                width: 180
                height: 180
        ...
        Item {
            ...
            Flickable {
                ...
                Column {
                    ...
                    // BUG FIX: removed `anchors.horizontalCenter` — it's a
                    // no-op when width equals parent.width, and QML emits
                    // "anchors may conflict" warnings.
                }
            }
        }
    }
