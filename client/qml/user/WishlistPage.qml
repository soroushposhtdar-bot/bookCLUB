...
import "../components"
// BUG FIX: missing import — LibraryService is a QML singleton registered in
// BookClub.Services. Without this import, `LibraryService.refresh()` and the
// `Connections { target: LibraryService }` block throw ReferenceError at
// runtime, and the wishlist never refreshes from the server.
...
Item {
    ...
    // Issue 7 — local view-mode property that drives which layout (grid / list)
    // is visible. The ViewToggle binds to this property; toggling it updates
    // both `_viewMode` (for visibility) and the viewModel (for persistence).
    // Previously the page bound ViewToggle.mode directly to
    // `root.viewModel.viewMode` and used `onModeChanged: function(mode) {…}` —
    // but the auto-generated `modeChanged` signal carries no parameter, so
    // `mode` was undefined and the toggle silently no-op'd.
    property string _viewMode: root.viewModel ? root.viewModel.viewMode : "grid"

    // Keep `_viewMode` in sync if the viewModel's viewMode changes externally
    // (e.g. restored from settings on a later page load).
    Connections {
        target: root.viewModel
        ignoreUnknownSignals: true
        function onViewModeChanged() {
            if (root.viewModel && root.viewModel.viewMode !== root._viewMode) {
                root._viewMode = root.viewModel.viewMode
            }
        }
    }
                    ...
                    Row {
                        ...
                        spacing: Theme.space.md
                            ...
                            ViewToggle {
                                ...
                                mode: root._viewMode
                                onModeChanged: {
                                    root._viewMode = _viewToggle.mode
                                    if (root.viewModel) root.viewModel.viewMode = _viewToggle.mode
                                }
                            }
                        }
                        ...
                        Row {
                            ...
                            spacing: Theme.space.md
                                ...
                                SearchField {
                                    ...
                                    onTextEdited: function(newText) { if (root.viewModel) root.viewModel.searchQuery = newText }
                                }
                        ...
                        Item {
                            ...
                            Column {
                                ...
                                spacing: 0
                                    ...
                                    Grid {
                                        width: parent.width
                                        visible: root.viewModel && !root.viewModel.isEmpty && (root._viewMode === "grid")
                                        columns: root._gridColumns
                                        spacing: Theme.space.xl

                                        Repeater {
                                            model: root.viewModel && root._viewMode === "grid" ? root.viewModel.books : []
                                            delegate: Item {
                                    ...
                                    Column {
                                        width: parent.width
                                        visible: root.viewModel && !root.viewModel.isEmpty && (root._viewMode === "list")
                                        spacing: Theme.space.sm

                                        Repeater {
                                            model: root.viewModel && root._viewMode === "list" ? root.viewModel.books : []
                                            delegate: BookRow {
                                                width: parent.width
                                                book: modelData
    ...
    // Issue 13 — refresh the wishlist from the server when the page loads.
