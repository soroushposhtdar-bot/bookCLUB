...
Item {
    ...
    // MF-026: client-side pagination per tab. Reset to 20 whenever the
    // underlying list changes OR the active tab changes (so switching from
    // a 100-book "My Books" tab to the "Downloaded" tab doesn't keep the
    // _visibleCount at 100). A single counter is safe because only one
    // tab is visible at a time.
    property int _visibleCount: 20
    ...
    Flickable {
        ...
        // QA-113: attach a vertical ScrollBar so users on desktop can
        // see + drag the scroll position instead of relying on wheel
        // scroll only. (QtQuick.Controls already imported above.)
            ...
            Row {
                ...
                Row {
                    ...
                    Column {
                        spacing: 0
                        Text {
                            text: String(root.viewModel ? root.viewModel.savedCount : 0)
                            color: Theme.color.textPrimary
                            font.family: Theme.font.family
                            font.pixelSize: Theme.font.sizeBodyLarge
                            font.weight: Theme.font.weightBold
                    ...
                    Item {
                        ...
                        Column {
                            ...
                            Grid {
                                ...
                                Repeater {
                                    // MF-026: paginate client-side.
                                    model: root.viewModel ? root.viewModel.myBooks.slice(0, root._visibleCount) : []
                                    delegate: Column {
                                        ...
                                        // Offline-download toggle — persists via LibraryService.toggleDownloaded
                                        TextButton {
                            ...
                            // MF-026: "Load more" button for the My Books tab.
                            Item {
                                width: parent.width
                                height: Theme.space.xxl
                                visible: root.viewModel && root.viewModel.myBooksCount > root._visibleCount
                                PrimaryButton {
                                    anchors.horizontalCenter: parent.horizontalCenter
                                    anchors.verticalCenter: parent.verticalCenter
                                    text: "Load more (" + (root.viewModel.myBooks.length - root._visibleCount) + " more)"  // FIX-MED-017
                                    iconName: "expand_more"
                                    onClicked: root._visibleCount += 20
                                }
                            }
                        }
                    }
                    ...
                    Item {
                        ...
                        Column {
                            ...
                            Grid {
                                ...
                                Repeater {
                                    // MF-026: paginate client-side.
                                    model: root.viewModel ? root.viewModel.downloadedBooks.slice(0, root._visibleCount) : []
                                    delegate: BookCard {
                                        width: (parent.width - (root._gridColumns - 1) * parent.spacing) / root._gridColumns
                                        book: modelData
                                        showAddButton: false
                            ...
                            // MF-026: "Load more" button for the Downloaded tab.
                            Item {
                                width: parent.width
                                height: Theme.space.xxl
                                visible: root.viewModel
                                      && root.viewModel.downloadedBooks.length > root._visibleCount
                                PrimaryButton {
                                    anchors.horizontalCenter: parent.horizontalCenter
                                    anchors.verticalCenter: parent.verticalCenter
                                    text: "Load more (" + (root.viewModel.downloadedBooks.length - root._visibleCount) + " more)"
                                    iconName: "expand_more"
                                    onClicked: root._visibleCount += 20
                                }
                            }
    ...
    // Issue: refresh the library from the server when the page loads so the
    // tabs always show current data (purchased books, downloaded, shelves).
    Component.onCompleted: {
        if (root.viewModel && root.viewModel.libraryService) {
            root.viewModel.libraryService.refresh()
        }
        // QA-109: keyboard navigation — focus the page root so the
        // activeFocusOnTab chain starts cleanly. LibraryPage has no
        // primary text field on the main view (the InputFields are inside
        // the create-shelf dialog), so we focus the root and let the user
        // Tab through the tab bar / book cards / action buttons.
        root.forceActiveFocus()
    }

    // MF-026: reset pagination counter when the underlying list changes
    // OR when the active tab changes. Without this, a refresh that adds
    // more books wouldn't reveal them until the user clicks Load more.
    Connections {
        target: root.viewModel
        ignoreUnknownSignals: true
        function onMyBooksChanged() { root._visibleCount = 20 }
        function onDownloadedBooksChanged() { root._visibleCount = 20 }
        function onActiveTabChanged() { root._visibleCount = 20 }
    }
}
