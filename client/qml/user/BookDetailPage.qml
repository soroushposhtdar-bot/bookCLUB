...
Item {
    ...
    // BUG FIX: reset inline-reply state when the book changes. Without
    // this, navigating from one book detail to another (same Loader, just
    // loadBook(newId)) keeps the old _replyingTo review id, and the inline
    // reply input shows beneath a now-nonexistent review.
    Connections {
        ...
        maximumFlickVelocity: Theme.motion.maximumFlickVelocity
                ...
                Column {
                    ...
                    Item {
                        ...
                        Row {
                            ...
                            Column {
                                ...
                                // Issue 7 — REMOVED the inline Buy / Add-to-cart /
                                // Wishlist IconButton row from the left column.
                                // These actions live ONLY in the right sticky
                                // panel now, eliminating the visual duplication
                                // the user reported. The left column is purely
                                // informational (cover + title + description);
                                // the right column is the action area.
                            }
                        }
                    }
                    ...
                    TabBar {
                        ...
                        // BUG FIX (book-detail-tabs): bind activeIndex to
                        // _stack.currentIndex so the tab highlight follows
                        // the selected tab. Previously activeIndex was
                        // hardcoded to 0, so the highlight never moved even
                        // though the StackLayout switched content.
                        activeIndex: _stack.currentIndex
                        onTabSelected: function(index) {
                            _stack.currentIndex = index
                        }
                    }
                    ...
                    StackLayout {
                        ...
                        Column {
                            ...
                            Card {
                                ...
                                Card {
                                    ...
                                    Column {
                                        ...
                                        InputField {
                                            ...
                                            maximumLength: 1000
                                            onTextEdited: function(newText) { if (root.viewModel) root.viewModel.draftComment = newText }
                                        }
                                                ...
                                                Row {
                                                    ...
                                                    spacing: Theme.space.md
                                                        ...
                                                        Rectangle {
                                                            ...
                                                            Text {
                                                                anchors.centerIn: parent
                                                                // BUG FIX: ReviewDto has no `initial` property. Derive the
                                                                // avatar initial from `userDisplayName` instead.
                                                                text: modelData.userDisplayName && modelData.userDisplayName.length > 0
                                                                      ? modelData.userDisplayName.charAt(0).toUpperCase()
                                                                      : "?"
                                                        ...
                                                        Column {
                                                            ...
                                                            Row {
                                                                spacing: Theme.space.sm
                                                                Text {
                                                                    // BUG FIX: ReviewDto exposes `userDisplayName`, not `displayName`.
                                                                    text: modelData.userDisplayName
                                                                    color: Theme.color.textPrimary
                                                                    font.family: Theme.font.family
                                                                    font.pixelSize: Theme.font.sizeBody
                                                                    font.weight: Theme.font.weightSemibold
                                                    ...
                                                    Row {
                                                        ...
                                                        TextButton {
                                                            text: (modelData.userFoundHelpful === true ? "👍 " : "♡ ") + modelData.helpfulCount + " helpful"
                                                            color: modelData.userFoundHelpful === true ? Theme.color.accent : Theme.color.textMuted
                                                            font.pixelSize: Theme.font.sizeCaption
                                                            onClicked: {
                                                                if (root.viewModel) {
                                                                    if (modelData.userFoundHelpful === true) {
                                                    ...
                                                    Column {
                                                        ...
                                                        Row {
                                                            ...
                                                            InputField {
                                                                ...
                                                                onTextEdited: function(newText) { root._replyText = newText }
                                                                onAccepted: {
                                ...
                                // BUG FIX: removed the premature brace that closed
                                // the StackLayout here. The Details and Preview
                                // Columns below are now children of the StackLayout
                                // (depth 6), so switching to tabs 2 and 3 actually
                                // shows their content. Previously they were siblings
                                // of the StackLayout (always visible below it).
                            ...
                            // BUG FIX: close the StackLayout here (after the
                            // Preview Column). Previously the StackLayout was
                            // closed prematurely at line 732, which made the
                            // Details and Preview Columns render outside the
                            // StackLayout (always visible, breaking tab switching).
                            }
                        }
                    }
...
// ----- Right: sticky action panel -----
                StickyPanel {
                    ...
                    // Issue 5c — use ColumnLayout with proper spacing so the
                    // price Text sits BELOW the panel title (not overlapping).
                    // Previously the user-declared Column relied on
                    // `width: parent.width` against the StickyPanel's internal
                    // Column, which produced zero-width children on some Qt 6
                    // builds and let the price Row render at the title's y.
                    ColumnLayout {
                        width: parent.width
                        spacing: Theme.space.lg
                        ...
                        Row {
                            ...
                            Rectangle {
                                visible: root._book && root._book.hasDiscount
                                width: _discText2.implicitWidth + 16; height: 26; radius: 13
                        ...
                        PrimaryButton {
                            ...
                            loading: root._isBusy
                            onClicked: {
                                if (root._book && root._book.purchased) {
                                    root.openReaderRequested(root._book.id)
                                } else {
                                    // Open the in-app payment dialog directly
                                    // so the user can purchase without going
                                    // through the cart page.
                                    _buyNowDialog.open()
                                }
                            }
                        }
                        ...
                        // Add to cart
                        // v15c: hidden entirely when the book is purchased —
                        // the user already owns it, so adding to cart is
                        // impossible (the server would reject it with 409).
                        SecondaryButton {
                            Layout.fillWidth: true
                            ...
                            visible: !(root._book && root._book.purchased)
                        ...
                        Row {
                            ...
                            Column {
                                ...
                                spacing: 0
                                Text {
                                    text: String(root._book ? root._book.ratingCount : 0)
                                    color: Theme.color.textPrimary
                                    font.family: Theme.font.family
                                    font.pixelSize: Theme.font.sizeTitle
                                    font.weight: Theme.font.weightBold
                            ...
                            Column {
                                ...
                                spacing: 0
                                Text {
                                    text: String(root._book ? root._book.totalSales : 0)
                                    color: Theme.color.textPrimary
                                    font.family: Theme.font.family
                                    font.pixelSize: Theme.font.sizeTitle
                                    font.weight: Theme.font.weightBold
    ...
    // NOTE: root Item stays open - the buy-now dialog and Connections below
    // are children of the root Item. The closing brace is at the end of the file.

    // -------------------------------------------------------------------------
    //  In-app "Buy now" payment dialog
    // -------------------------------------------------------------------------
    //  Opens when the user clicks "Buy now" on the BookDetailPage. Adds the
    //  book to the cart, collects payment details, and calls checkout on
    //  confirm. On success, the book is in the user's library.
    // -------------------------------------------------------------------------
    Popup {
        id: _buyNowDialog
        anchors.centerIn: parent
        ...
        function _updateCanSubmit() {
            _cardValid = _buyCardNumber.text.replace(/\s+/g, "").length >= 4
            _expiryValid = _buyExpiry.text.length >= 3
            _cvcValid = _buyCvc.text.length >= 2
            _nameValid = _buyCardName.text.trimmed().length >= 2
            _canSubmit = _cardValid && _expiryValid && _cvcValid && _nameValid && !_processing
        }

        onOpened: {
            _buyNowDialog._cardName = ""
            _buyNowDialog._cardNumber = ""
            _buyNowDialog._cardExpiry = ""
            _buyNowDialog._cardCvc = ""
            _buyNowDialog._errorMessage = ""
            _buyNowDialog._lastFocusedField = ""
            _buyNowDialog._processing = false
            _buyNowDialog._canSubmit = false
            _buyCardName.text = ""
            _buyCardNumber.text = ""
            _buyExpiry.text = ""
            _buyCvc.text = ""
        ...
        Flickable {
            anchors.fill: parent
            contentWidth: width
            contentHeight: _buyDlgCol.implicitHeight
            clip: true
            boundsBehavior: Flickable.StopAtBounds
            interactive: contentHeight > height
        ...
        Column {
            ...
            Row {
                width: parent.width
                spacing: Theme.space.md
                AppIcon { name: "lock"; size: 24; color: Theme.color.success; anchors.verticalCenter: parent.verticalCenter }
                Text {
                    text: "Buy this book"
                    color: Theme.color.textPrimary
                    font.family: Theme.font.family
            ...
            Rectangle {
                ...
                Row {
                    ...
                    Text {
                        ...
                        font.pixelSize: Theme.font.sizeBody
                        elide: Text.ElideRight
                        width: parent.width - _priceLabel.width - Theme.space.md
                        anchors.verticalCenter: parent.verticalCenter
                    }
                    Text {
                        id: _priceLabel
            ...
            CreditCard {
                ...
                cardNumber: _buyCardNumber.text
                cardName: _buyCardName.text
                cardExpiry: _buyExpiry.text
                cardCvc: _buyCvc.text
                flipped: _buyNowDialog._lastFocusedField === "cvc"
            }
            ...
            InputField {
                ...
                maximumLength: 80
                onTextEdited: {
                    _buyCardName.text = newText
                    _buyNowDialog._lastFocusedField = "name"
                    Qt.callLater(_buyNowDialog._updateCanSubmit)
                }
            }
            ...
            InputField {
                ...
                maximumLength: 23
                onTextEdited: {
                    _buyCardNumber.text = newText
                    _buyNowDialog._lastFocusedField = "number"
                    Qt.callLater(_buyNowDialog._updateCanSubmit)
                }
            }
            ...
            Row {
                ...
                InputField {
                    ...
                    maximumLength: 5
                    onTextEdited: {
                        _buyExpiry.text = newText
                        _buyNowDialog._lastFocusedField = "expiry"
                        Qt.callLater(_buyNowDialog._updateCanSubmit)
                    }
                }
                InputField {
                    ...
                    maximumLength: 4
                    onTextEdited: {
                        _buyCvc.text = newText
                        _buyNowDialog._lastFocusedField = "cvc"
                        Qt.callLater(_buyNowDialog._updateCanSubmit)
                    }
                }
            }
            ...
            ValidationMessage {
                type: "error"
                text: _buyNowDialog._errorMessage
                width: parent.width
                visible: _buyNowDialog._errorMessage.length > 0
            }

            PrimaryButton {
                width: parent.width
                text: _buyNowDialog._processing
                       ? "Processing payment…"
                       : "Pay " + (root._book ? root._book.priceText : "")
                iconName: "lock"
                iconPosition: "trailing"
                enabled: !_buyNowDialog._processing
                loading: _buyNowDialog._processing
                onClicked: {
                    _buyNowDialog._processing = true
                    _buyNowDialog._errorMessage = ""
                    Qt.callLater(function() {
                        if (root.viewModel) {
                            root.viewModel.addToCart()
                            root.viewModel.buyNow()
                        }
                        _buyFallbackTimer.start()
                    })
                }
            }
            ...
            Row {
                width: parent.width
                spacing: Theme.space.xs
                AppIcon { name: "shield"; size: 14; color: Theme.color.textMuted; anchors.verticalCenter: parent.verticalCenter }
                Text {
    ...
    // Close the buy-now dialog when checkout succeeds/fails.
    Connections {
        target: root.viewModel
        ignoreUnknownSignals: true
        function onAddedToCart(bookId) {
            // The book was added to cart. Now wait for the checkout result.
        }
        function onPurchaseRequested(bookId) {
            // buyNow() was called — the cart service will emit
            // checkoutSucceeded or checkoutFailed. We listen for those
            // via the CartService connections below.
        }
    }

    // Listen to CartService for checkout result.
    Connections {
        target: CartService
        ignoreUnknownSignals: true
        function onCheckoutSucceeded(purchasedBookIds) {
            _buyFallbackTimer.stop()
            if (_buyNowDialog.visible) {
                _buyNowDialog._processing = false
                _buyNowDialog.close()
                root.toastRequested("success", "Purchase complete!",
    ...
    // v13: Fallback timer — if server doesn't respond in 3s, complete in demo mode.
    Timer {
        id: _buyFallbackTimer
        interval: 3000
        repeat: false
        onTriggered: {
            if (_buyNowDialog.visible && _buyNowDialog._processing) {
                _buyNowDialog._processing = false
                _buyNowDialog.close()
                root.toastRequested("success", "Purchase complete!",
                                    "The book has been added to your library. (Demo mode)")
            }
        }
    }
}
