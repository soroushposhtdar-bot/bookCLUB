...
Item {
    ...
    // BUG FIX: the Flickable's children used `anchors.leftMargin/rightMargin`
    // without `anchors.left/right` — the margins were silently ignored and
    // the content touched the screen edges. Replaced with explicit `x` +
    // `width` calculations so the padding is always applied.
    Flickable {
                                ...
                                Column {
                                    ...
                                    Row {
                                        ...
                                        InputField {
                                            ...
                                            onTextEdited: if (root.viewModel) root.viewModel.downloadLocation = text
                                        }
                                        SecondaryButton {
                                            id: _browseBtn
                                            text: "Browse"
    ...
    // Folder/file picker stubs (dialogs removed for compatibility)
    Item { visible: false; id: _folderDialog; function open() { root.toastRequested("info", "Browse", "Type a download path in the field above.") } }

    Item { visible: false; id: _avatarDialog; function open() { root.toastRequested("info", "Avatar", "Avatar upload is not available in this build.") } }
}