...
Item {
    ...
    // -------------------------------------------------------------------------
    // BUG FIX (Issue 4): the C++ signal is `realtimeNotificationReceived(QObject* dto)`,
    // so the handler parameter MUST be named `dto` to match. The previous
    // code referenced `notification` (undefined in this scope — that name
    // only exists on the page's own re-emitted signal), so App.qml always
    // received `undefined` and no real-time toast was ever shown.
    Connections {
        target: root.viewModel
        ignoreUnknownSignals: true
        onRealtimeNotificationReceived: function(dto) {
            root.realtimeNotificationReceived(dto)
        }
    }

    // Issue: refresh notifications from the server when the page loads.
    Component.onCompleted: {
        if (root.viewModel) {
            // Make sure the service is attached before refreshing.
            if (root.viewModel.service) {
                root.viewModel.service.refresh()
            }
            // Reset to the "all" category on entry so the user sees the
            // full list before filtering by topic.
            root.viewModel.setActiveCategory("all")
        }
    }
}
