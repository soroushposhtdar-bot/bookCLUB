...
MultiEffect {
    ...
    // Shadow configuration — only use properties that exist in all
    // MultiEffect versions (Qt 6.5+). `shadowBlur` accepts a normalized
    // 0..1 value; we don't set `shadowBlurMax` because it was added in a
    // later Qt 6.x minor version and may not exist.
    shadowEnabled: true
