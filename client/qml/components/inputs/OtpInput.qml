...
FocusScope {
    ...
    function setValue(v) {
        ...
        for (var i = 0; i < root.length; i++) {
            _repeater.itemAt(i).charValue = i < safe.length ? safe[i] : ""
        }
        _recheckComplete()
    }
    ...
    function forceActiveFocus() {
        ...
        for (var i = 0; i < root.length; i++) {
            ...
            if (cell && cell.charValue.length === 0) { cell.activate(); return }
        }
        if (_repeater.count > 0) _repeater.itemAt(0).activate()
    }
                ...
                TextField {
                    ...
                    onTextChanged: {
                        if (text.length > 1) {
                            ...
                            for (var i = 0; i < pasteText.length && idx + i < root.length; i++) {
                                ...
                                if (cell) cell.charValue = pasteText[i]
                            }
                        ...
                        if (text === _cell.charValue) return
                        _cell.charValue = text
                        // Rebuild root.value
                        var v2 = ""
                        for (var k = 0; k < root.length; k++) {
                            var c2 = _repeater.itemAt(k)
                            if (c2) v2 += c2.charValue
                        }
                        root.value = v2
                        root.edited(v2)
                    ...
                    Keys.onPressed: {
                        if (event.key === Qt.Key_Backspace && _tf.text.length === 0 && index > 0) {
                            ...
                            if (prev) { prev.charValue = ""; prev.activate() }
                            event.accepted = true
