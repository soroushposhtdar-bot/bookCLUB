...
Item {
    ...
    Column {
        ...
        InputField {
            ...
            onTextEdited: {
                ...
                // the external binding (e.g. `text: root.viewModel.password`).
                // Just emit the signal; the consumer updates the VM, which
                // updates `root.text`, which InputField's internal Connections
                // block pushes to the TextField.
    ...
    // Just emit the signal via the core InputField's clear().
    function clear() { _core.clear() }
}
