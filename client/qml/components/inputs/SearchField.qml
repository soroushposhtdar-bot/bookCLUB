...
Item {
    ...
    Row {
        ...
        TextField {
            ...
            onTextEdited: {
                ...
                // the external binding. Just emit the signal; the consumer
                // updates the VM, which updates `root.text`, and the
                // Connections block below pushes the new value to the TextField.
                root.textEdited(text)
            }
            onAccepted: root.accepted()

            // BUG FIX: sync external `root.text` changes to the TextField
            // (e.g. when the consumer clears the query via VM).
            Connections {
                target: root
                function onTextChanged() {
                    if (_tf.text !== root.text) {
                        _tf.text = root.text
                    }
                }
            }
        ...
        IconButton {
            ...
            onClicked: {
                // BUG FIX: don't write to `root.text` or `_tf.text` —
                // destroys the external binding. Just emit the signal;
                // the consumer clears the VM, which updates `root.text`,
                // and the Connections block pushes to the TextField.
                root.textEdited("")
    ...
    // BUG FIX: don't write to `root.text` — destroys the external binding.
    function clear() { textEdited("") }
}
