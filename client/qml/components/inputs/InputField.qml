...
Item {
            ...
            Row {
                ...
                TextField {
                    ...
                    width: parent.width - _row.spacing * 2
                            ...
                            - (_trailingItem.width)
                    height: parent.height
                    // BUG FIX: previously `text: root.text` + `onTextEdited: root.text = text`.
                    // The `root.text = text` assignment destroys the external binding on
                    // `root.text` (e.g. `text: root.viewModel.username`), so programmatic
                    // VM changes stopped updating the field after the first keystroke.
                    // We now display `root.text` only when the VM pushes a new value
                    // (via the Connections block below), and `onTextEdited` only emits
                    // the signal — the consumer updates the VM, which then updates
                    // `root.text`, which the Connections block pushes to the TextField.
                    text: root.text
                    ...
                    onTextEdited: {
                        // BUG FIX: don't write to `root.text` — that would destroy
                        // the external binding (e.g. `text: root.viewModel.username`).
                        // Just emit the signal; the consumer updates the VM, which
                        // updates `root.text`, and the Connections block below pushes
                        // the new value to the TextField.
                    ...
                    // BUG FIX: when `root.text` changes externally (e.g. the VM
                    // is updated by the consumer's onTextEdited handler, or by
                    // loadSavedCredentials, or by reset()), push the new value
                    // to the TextField — but only if it differs from the
                    // TextField's current text, to avoid cursor jumps while typing.
                    Connections {
                        target: root
                        function onTextChanged() {
                            if (_textField.text !== root.text) {
                                var pos = _textField.cursorPosition
                                _textField.text = root.text
                                // Restore cursor position (clamped to new length)
                                _textField.cursorPosition = Math.min(pos, root.text.length)
                            }
                        }
                    }
                }
                    ...
                    IconButton {
                        ...
                        onClicked: {
                            if (root.showClearButton && root.text.length > 0) {
                                // BUG FIX: don't write to `root.text` or
                                // `_textField.text` directly — that destroys
                                // the external binding. Just emit the signal;
                                // the consumer clears the VM, which updates
                                // `root.text`, and the Connections block
                                // pushes the empty string to the TextField.
                                root.textEdited("")
                            } else {
    ...
    // BUG FIX: don't write to `text` or `_textField.text` directly — that
    // destroys the external binding. Just emit the signal; the consumer
    // clears the VM, which updates `root.text`, and the Connections block
    // pushes the empty string to the TextField.
    function clear() {
        textEdited("")
    }
