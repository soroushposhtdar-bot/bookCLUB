...
Item {
    ...
    MouseArea {
        ...
        onClicked: {
            // BUG FIX: previously did `root.checked = !root.checked` which
            // destroys any external binding on `checked` (e.g.
            // `checked: root.viewModel ? root.viewModel.rememberMe : false`).
            // After the first click, programmatic changes to the VM no longer
            // updated the checkbox UI. We now only emit the `toggled` signal
            // and let the consumer update the VM — the binding then pushes
            // the new value back into `checked`.
    ...
    // BUG FIX: previously did `checked = !checked` which destroys any
    // external binding on `checked`. Now only emits the signal — the
    // consumer updates the VM, which pushes back via the binding.
    function toggle() {
        if (enabled) {
            toggled(!checked)
        }
