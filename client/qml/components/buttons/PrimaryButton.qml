...
Button {
    ...
    // BUG FIX: removed `enabled: parent ? true : true` — dead ternary that
    // always evaluates to true and clobbers external `enabled:` bindings.
    // Button.enabled defaults to true; external assignments now work.
    ...
    background: Item {
        ...
        Rectangle {
            ...
            color: !control.enabled ? Qt.rgba(Theme.color.primary.r, Theme.color.primary.g, Theme.color.primary.b, 0.35)
        ...
        // BUG FIX: removed `scale: control.pressed ? 0.985 : 1.0` from the
        // background — it double-scaled with the `transform: Scale` below,
        // causing subtle jitter on press. The transform handles the press
        // animation for the whole button (background + content).
    }
