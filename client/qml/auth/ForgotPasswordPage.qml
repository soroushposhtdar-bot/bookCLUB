...
Item {
    ...
    AuthLayout {
        ...
        // `Layout.fillWidth` spacer actually fills.
        RowLayout {
            width: parent.width
            IconButton {
                iconName: "arrow_back"
        ...
        InputField {
            ...
            onAccepted: if (_continueBtn.enabled) _continueBtn.clicked()
        ...
        // when the parent RowLayout/Row has `width: parent.width`.
        TextButton {
            text: "Back to login"
            iconName: "arrow_back"
    ...
    // BUG FIX (Issue 17): the answer field's `Component.onCompleted`
    // only fired once at page creation (when step === "username"). When
    // the user advanced to the answer step, the answer field never
    // received focus. This Connections block re-focuses the answer
    // field whenever the VM transitions to the "answer" step.
    // We use Qt.callLater because the answer Column's `visible` binding
    // re-evaluates AFTER the stepChanged signal fires — forceActiveFocus
    // on an invisible item is a no-op, so we defer to the next event
    // loop iteration when the Column is already visible.
    // BUG FIX: Qt 6 requires `function(params)` syntax for parameterized
    // signal handlers. `stepChanged(const QString& step)` is parameterized.
    Connections {
        target: root.viewModel
        ignoreUnknownSignals: true
        function onStepChanged(step) {
            if (step === "answer") Qt.callLater(function() { _answer.forceActiveFocus() })
        }
    }
