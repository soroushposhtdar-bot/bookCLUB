...
//     component's totalPages binding re-evaluates when viewModel.sessions changes
//   - Added fallback Connections handlers (onRefreshed, onDataChanged)
//
// LAYOUT FIXES:
//   - FIX 2-A: Added top spacer + switched KPI row to RowLayout
//   - FIX 2-B: All Card inner Columns: anchors.fill → width-only;
//     ListViews use content-driven height (Math.max(emptyHeight, count * rowHeight));
//     Pagination Row → RowLayout with explicit height + spacer
...
Item {
    ...
    // picks up changes to viewModel.sessions and re-evaluates totalPages.
    readonly property int _totalPages: Math.max(1, Math.ceil((page.viewModel ? page.viewModel.sessions.length : 0) / page._pageSize))
    ...
    Connections {
        ...
        function onSessionsChanged() { page._refreshPaged() }
    ...
    ScrollView {
        ...
        Column {
            ...
            // FIX 2-A — Top breathing room
            Item { width: 1; height: Theme.space.xl }

            // ----- KPIs (FIX 2-A: RowLayout) -----
            RowLayout {
                width: parent.width
            ...
            Card {
                ...
                // FIX 2-B — width-only (NOT anchors.fill)
                Column {
                    width: parent.width
                    ...
                    ListView {
                        width: parent.width
                        // FIX 2-B — content-driven height
                        height: Math.max(
                            page._pagedSessions.length === 0 ? 200 : 0,
                            page._pagedSessions.length * 52
                        )
                        clip: true
                        interactive: false
                        spacing: 0
            ...
            Card {
                ...
                Column {
                    ...
                    ListView {
                        ...
                        // FIX 2-B — content-driven height
                        height: Math.max(
                            (page.viewModel ? page.viewModel.rooms.length : 0) === 0 ? 160 : 0,
                            (page.viewModel ? page.viewModel.rooms.length : 0) * 48
                        )
                        clip: true
                        interactive: false
                        spacing: 0
