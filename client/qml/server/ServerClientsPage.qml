...
//
// FUNCTIONAL FIXES:
//   - Converted _totalPages() function → readonly property so the Pagination
//     component's totalPages binding re-evaluates when viewModel.clients changes
//   - Added fallback Connections handlers (onRefreshed, onDataChanged)
//
// LAYOUT FIXES:
//   - FIX 1-A: Added top spacer + switched KPI row to RowLayout with
//     Layout.fillWidth + Layout.preferredWidth: 1 on each StatCard
//   - FIX 1-B: Card inner Column: anchors.fill → width-only; ListView height
//     is now content-driven via Math.max(emptyHeight, count * rowHeight)
//   - FIX 1-C: Pagination Row → RowLayout with explicit height + spacer
...
Item {
    ...
    // Readonly property (not a function) so QML binding dependency tracking
    // picks up changes to viewModel.clients and re-evaluates totalPages.
    readonly property int _totalPages: Math.max(1, Math.ceil((page.viewModel ? page.viewModel.clients.length : 0) / page._pageSize))

    function _refreshPaged() {
        var raw = page.viewModel ? page.viewModel.clients : []
        // Normalize server fields → QML display fields.
        // Server sends: { id, userId, authenticated, connected }
        // QML expects:  { clientId, user, role, ip, since, latency }
        var all = []
        for (var j = 0; j < raw.length; ++j) {
            var c = raw[j]
            all.push({
                clientId: c.id        || c.clientId  || "—",
                user:     c.userId    || c.user       || "anonymous",
                role:     c.role      || "user",
                ip:       c.ip        || c.ipAddress  || "—",
                since:    c.since     || c.connectedAt|| "—",
                latency:  c.latency   !== undefined ? c.latency + " ms" : "—",
                connected: c.connected !== false
            })
        }
        var start = (page._currentPage - 1) * page._pageSize
        if (start >= all.length) { page._currentPage = 1; start = 0 }
        var end = Math.min(start + page._pageSize, all.length)
        page._pagedClients = all.slice(start, end)
    }
    ...
    Connections {
        ...
        function onClientsChanged() { page._refreshPaged() }
        // Fallback handlers for VMs that emit a single "refreshed" / "dataChanged"
        // signal instead of granular per-property signals.
        function onRefreshed() { page._refreshPaged() }
        function onDataChanged() { page._refreshPaged() }
        function onDataReady() { page._refreshPaged() }
    }
    ...
    ScrollView {
        ...
        Column {
            ...
            // FIX 1-A — Top breathing room
            Item { width: 1; height: Theme.space.xl }

            // ----- KPI cards (FIX 1-A: RowLayout) -----
            RowLayout {
                width: parent.width
            ...
            Card {
                ...
                // FIX 1-B — width-only (NOT anchors.fill) so children drive height
                Column {
                    width: parent.width
                    ...
                    ListView {
                        width: parent.width
                        // FIX 1-B — content-driven height (was: page._pagedClients.length * 52)
                        height: Math.max(
                            page._pagedClients.length === 0 ? 240 : 0,
                            page._pagedClients.length * 52
                        )
                        clip: true
                        interactive: false
                        spacing: 0
                    ...
                    // Pagination (FIX 1-C — RowLayout with explicit height + spacer)
                    RowLayout {
                        width: parent.width
                        height: 40
