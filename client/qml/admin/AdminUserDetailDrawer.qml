...
//     so the user detail reloads whenever the VM pushes new data
//   - Defensive null/undefined check in _reload
...
Item {
    ...
    // ----- Role → color map -----
    // BUG FIX (admin polish): the server returns `role` as an integer
    // (0=User, 1=Publisher, 2=Admin) AND `roleName` as a string. The
    // previous helpers only accepted string roles, so the role badge
    // always fell through to the default (accent) color. Now we
    // normalize the input: accept either the integer or the string
    // form, and also accept the roleName string.
    ...
    Connections {
        ...
        function onUsersChanged() { drawer._reload() }
        ...
        ColumnLayout {
            ...
            ScrollView {
                ...
                ColumnLayout {
                                ...
                                Row {
                                    ...
                                    Rectangle {
                                        ...
                                        Text {
                                            id: _roleLbl
                                            anchors.centerIn: parent
                                            // BUG FIX (admin polish): use _roleLabel() so the
                                            // badge shows "Admin" / "Publisher" / "User" instead
                                            // of the raw integer 0/1/2 from the server.
                                            text: drawer._roleLabel(drawer._detail.role || "user")
                    ...
                    // ----- (Memberships section removed — not in requirements doc) -----
                    // The Memberships feature was over-scoped UI with no backend
                    // support and no mention in the project requirements. Hidden
                    // to avoid showing an empty "No memberships" state for every user.
                    Card {
                        visible: false  // BUG FIX (major #2.3): hidden — not in requirements
                        width: parent.width - 2 * Theme.space.xl
                        anchors.horizontalCenter: parent.horizontalCenter
                        elevation: "none"
                        bordered: true
                        ...
                        ColumnLayout {
                                ...
                                delegate: Rectangle {
                                    ...
                                    RowLayout {
                                        ...
                                        IconButton {
                                            ...
                                            onClicked: {
                                                ...
                                                if (modelData.status === "Active") {
                                                    if (typeof drawer.viewModel.suspendMembership === "function") {
                                                    ...
                                                    if (typeof drawer.viewModel.reactivateMembership === "function") {
                                        ...
                                        IconButton {
                                            ...
                                            anchors.verticalCenter: parent.verticalCenter
                                            onClicked: {
                                                if (drawer.viewModel && typeof drawer.viewModel.cancelMembership === "function") {
                            ...
                            EmptyState {
                                width: parent.width
                                height: (drawer._detail.memberships || []).length === 0 ? 100 : 0
                    ...
                    Card {
                        ...
                        Column {
                            ...
                            Row {
                                ...
                                Repeater {
                                    // BUG FIX (admin polish): removed "server" from the
                                    // role options — the AccountRole enum only has User /
                                    // Publisher / Admin. The server console uses a separate
                                    // login (the ServerOperator role), not a user-account
                                    // role, so offering it here was misleading.
                                    model: ["user", "publisher", "admin"]
                                    FilterChip {
                                        label: modelData
                                        // BUG FIX (admin polish): compare normalized role
                                        // keys so the checkmark shows on the correct chip
                                        // when the server returns an integer role.
                                        iconName: drawer._roleKey(drawer._detail.role || "user") === modelData ? "check" : ""
                                        onClicked: {
                            ...
                            Row {
                                ...
                                PrimaryButton {
                                    ...
                                    onClicked: {
                                        ...
                                        if (drawer._detail.status === "Active") {
                                            if (typeof drawer.viewModel.blockUser === "function") {
                                            ...
                                            if (typeof drawer.viewModel.unblockUser === "function") {
                                ...
                                SecondaryButton {
                                    text: drawer._detail.status === "Active" ? "Deactivate" : "Activate"
                                    iconName: drawer._detail.status === "Active" ? "block" : "check_circle"
                                    enabled: drawer.username.length > 0
                                    onClicked: {
                                        if (!drawer.viewModel) return
                                        if (drawer._detail.status === "Active") {
                                            if (typeof drawer.viewModel.deactivateUser === "function") {
                                                drawer.viewModel.deactivateUser(drawer.username)
                                                drawer.toastRequested("info", "User deactivated",
                                                                      "@" + drawer.username + " is now inactive.")
                                            }
                                        } else {
                                            if (typeof drawer.viewModel.activateUser === "function") {
                                                drawer.viewModel.activateUser(drawer.username)
                                                drawer.toastRequested("success", "User activated",
                                                                      "@" + drawer.username + " is now active.")
                                            }
                                        }
                                    }
                                }
