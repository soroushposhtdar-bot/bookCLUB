...
void PublisherViewModel::refresh() { beginAsync(10); }
...
bool PublisherViewModel::addPromotion(const QString& code, const QString& description, int discountPercent, int cap, const QString& startDate, const QString& endDate) {
    ...
    QString result = m_service->addPromotion(code, description, discountPercent, cap, startDate, endDate);
    return !result.isEmpty();
}
bool PublisherViewModel::updatePromotion(const QString& code, const QString& description, int discountPercent, const QString& startDate, const QString& endDate) {
    if (!m_service) return false;
    return m_service->updatePromotion(code, description, discountPercent, startDate, endDate);
}
