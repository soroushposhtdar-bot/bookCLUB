...
//  ServerViewModel.cpp
// =============================================================================
//  Thin pass-through to ServerService. Listens to ServerService::dataChanged
//  and re-emits the granular signals so the QML ServerShell + pages refresh.
...
void ServerViewModel::setServerService(ServerService* s) {
    ...
    m_service = s;
    if (m_service) {
        // Re-broadcast service signals to QML-facing VM signals.
        connect(m_service, &ServerService::dataChanged, this, [this]() {
            emit clientsChanged();
            emit sessionsChanged();
            emit logsChanged();
            emit servicesChanged();
        });
        connect(m_service, &ServerService::logsChanged, this, [this]() {
            emit logsChanged();
        });
        connect(m_service, &ServerService::clientsChanged, this, [this]() {
            emit clientsChanged();
        });
    }
...
int ServerViewModel::cpuLoad() const { return m_service ? static_cast<int>(m_service->cpuLoad()) : 0; }
int ServerViewModel::ramUsage() const { return m_service ? static_cast<int>(m_service->ramUsage()) : 0; }
int ServerViewModel::diskUsage() const { return m_service ? static_cast<int>(m_service->diskUsage()) : 0; }

void ServerViewModel::refresh() { if (m_service) m_service->refresh(); beginAsync(400); }
void ServerViewModel::onAsyncReady() {
    emit clientsChanged(); emit sessionsChanged(); emit logsChanged(); emit servicesChanged();
    finishAsync();
}
...
QVariantList ServerViewModel::filterLogs(const QString& level, const QString& search) const {
