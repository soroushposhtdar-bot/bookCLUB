...
QString ReaderViewModel::bookAuthor() const { return m_readerService ? m_readerService->authorName() : QString{}; }
// v15j: cover getters for BookCover.
QString ReaderViewModel::coverColor()  const { return m_readerService ? m_readerService->coverColor()  : QString{}; }
QString ReaderViewModel::coverAccent() const { return m_readerService ? m_readerService->coverAccent() : QString{}; }
QString ReaderViewModel::coverImage()  const { return m_readerService ? m_readerService->coverImage()  : QString{}; }
...
QString ReaderViewModel::pageText() const {
    ...
    // BUG FIX (Issue 5): guard against p < 1. The previous version
    // computed `(p - 1) % openers.size()` directly; if `p` was 0 (the
    // old initial value in ReaderService::openBook), this yielded
    // `-1 % 5 = -1` in C++ → `openers.at(-1)` → undefined behavior /
    // crash. We now clamp p to >= 1 before computing the indices.
    const int safePage = p < 1 ? 1 : p;
    ...
    const int openerIdx = (safePage - 1) % openers.size();
    const int bodyIdx    = (safePage - 1) % bodies.size();
    const int closerIdx  = (safePage - 1) % closers.size();
...
void ReaderViewModel::openBook(const QString& bookId) {
    if (!m_readerService) {
        setError(QStringLiteral("Reader service is not available."));
        return;
    }
    clearError();
    ...
    QMetaObject::invokeMethod(this, [this, bookId]() {
        ...
        emit loadingChanged();
        if (!ok) {
            setError(QStringLiteral("Couldn't open this book. It may have been removed."));
            return;
        }
        m_zoom = 1.0;
        m_fitWidth = true;
...
// v15: setPageCount — called from QML when the PdfDocument reports its
// real page count. Delegates to the service so the page count flows
// through the normal notification path (currentBookChanged) and all
// QML bindings on pageCount refresh.
void ReaderViewModel::setPageCount(int count) {
    if (m_readerService) m_readerService->setPageCount(count);
}
...
// v15: toggleBookmark(int page) — toggles the bookmark on a SPECIFIC page
// (not the current page). This is what the reader page's bookmark button
// and the sidebar page list call, so they can bookmark the page the user
// is actually looking at (which may differ from viewModel.page when the
// PDF view scrolled ahead of the VM's page cursor).
void ReaderViewModel::toggleBookmark(int page) {
    if (!m_readerService || page < 1) return;
    m_readerService->toggleBookmark(page);
}
...
// v12: openExternally — opens the PDF in the system's default viewer.
// This is a fallback for when the internal reader can't render the actual
// PDF file (e.g. the file path is on a different machine).
bool ReaderViewModel::openExternally() const {
    if (!m_readerService) return false;
    const QString path = m_readerService->currentPdfPath();
    if (path.isEmpty()) return false;
    // Build a file:// URL (handle Windows drive letters)
    QUrl url;
    if (path.startsWith('/')) {
        url = QUrl("file://" + path);
    } else {
...
QString ReaderViewModel::pdfFilePath() const {
    return m_readerService ? m_readerService->currentPdfPath() : QString{};
}

// v13: pdfUrl — wraps the local pdfFilePath in a QUrl so QtQuick.Pdf's
// PdfDocument.source can consume it directly. Returns an empty QUrl when
// there is no PDF available, which the QML uses as a signal to fall back
// to the synthetic text view.
QUrl ReaderViewModel::pdfUrl() const {
    if (!m_readerService) return {};
    const QString path = m_readerService->currentPdfPath();
