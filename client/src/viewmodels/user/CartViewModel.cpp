...
double CartViewModel::purchaseProfit() const {
    return m_cartService ? m_cartService->purchaseProfit() : 0.0;
}
QString CartViewModel::purchaseProfitText() const {
    return m_cartService ? m_cartService->purchaseProfitText() : QStringLiteral("$0.00");
}
...
void CartViewModel::checkout() {
    ...
    beginAsync(10);   // simulate order-creation round-trip
}
