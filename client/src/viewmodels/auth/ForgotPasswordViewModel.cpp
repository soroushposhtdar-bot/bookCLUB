...
void ForgotPasswordViewModel::_doSubmit() {
    ...
    // Stash the security answer so ResetPasswordViewModel can forward it
    // to the server when the user submits the new password.
    authService()->stashSecurityAnswer(m_securityAnswer);
