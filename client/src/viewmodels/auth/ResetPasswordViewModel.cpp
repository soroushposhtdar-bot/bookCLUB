...
void ResetPasswordViewModel::setUsername(const QString& v) {
    ...
    // BUG FIX: recompute canSubmit — _computeCanSubmit() checks
    // !m_username.isEmpty(), so after the router sets the username the
    // Continue button's enabled state should update immediately.
    _recomputeCanSubmit();
}
