...
// v22 (Issue 5): setSelectedRole — accepts "user" or "publisher". Any
// other value is normalised to "user" (defensive — QML might pass garbage).
void RegisterViewModel::setSelectedRole(const QString& v) {
    QString normalised = v.trimmed().toLower();
    if (normalised != QStringLiteral("publisher")) {
        normalised = QStringLiteral("user");
    }
    if (m_selectedRole == normalised) return;
    m_selectedRole = normalised;
    emit selectedRoleChanged(m_selectedRole);
    _recomputeCanSubmit();
}
...
void RegisterViewModel::_doSubmit() {
    ...
    QString error;
    // v22 (Issue 5): pass m_selectedRole through to AuthService so the
    // server's Register endpoint can create a Publisher-derived user
    // instead of a RegularUser when the user picked "publisher".
    if (authService()->registerUser(m_username, m_displayName, m_password,
                                     m_securityQuestion, m_securityAnswer,
                                     m_selectedRole, error)) {
        _finishMockedOperation();
...
void RegisterViewModel::_doReset() {
    ...
    m_acceptTerms = false;
    m_selectedRole = QStringLiteral("user");  // v22 (Issue 5): reset to default.
