...
class RegisterViewModel : public AuthViewModelBase {
    ...
    // v22 (Issue 5): the role the user picked on the register form.
    // Either "user" (default) or "publisher". Passed through to
    // AuthService::registerUser → server's Register endpoint.
...
public:
    ...
    // v22 (Issue 5): "user" (default) or "publisher".
    const QString&  selectedRole() const { return m_selectedRole; }
...
public slots:
    ...
    // v22 (Issue 5): accepts "user" or "publisher". Any other value
    // (including empty) is normalised to "user". Emits selectedRoleChanged
    // only when the value actually changes.
    Q_INVOKABLE void setSelectedRole(const QString& v);
...
signals:
    ...
    // v22 (Issue 5): emitted when selectedRole changes.
    void selectedRoleChanged(const QString& v);
...
private:
    ...
    // v22 (Issue 5): "user" or "publisher". Default "user".
    QString m_selectedRole = QStringLiteral("user");
