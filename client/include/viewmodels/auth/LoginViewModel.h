...
public:
    ...
    // Load saved credentials from QSettings (called on startup).
    Q_INVOKABLE void loadSavedCredentials();
    // Save credentials to QSettings (called on successful login with rememberMe=true).
    Q_INVOKABLE void saveCredentials();
    // Clear saved credentials (called when rememberMe is unchecked).
...
private:
    ...
    bool m_loadingSaved = false;  // guard to prevent setRememberMe from clearing during load
};
