...
public slots:
    ...
    // BUG FIX (profile-empty): explicit refresh method that re-fetches the
    // user's purchase history + favorite genres from the server and caches
    // them so the Q_PROPERTY reads don't each trigger a blocking network
    // call. Called from ProfilePage.qml's Component.onCompleted.
...
private:
    ...
    // BUG FIX (Issue 3): flag that tracks whether the user has started
    // editing the displayName field. Before the first edit, displayName()
    // falls back to UserService; after the first edit, it returns the
    // local cache so the binding doesn't revert keystrokes.
    bool m_displayNameEdited = false;
    ...
    // BUG FIX (profile-empty): cached purchase history so Q_PROPERTY reads
    // don't each trigger a blocking network call. Populated by refresh().
    QList<QObject*> m_purchaseHistory;
};
