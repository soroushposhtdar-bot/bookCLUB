...
signals:
    ...
    // BUG FIX (Issue 19): added so the VM can report a failed create to
    // the UI (instead of always emitting shelfCreated, which made
    // UserShell.qml show a green "Shelf created" toast even on failure).
    void shelfCreateFailed(const QString& error);
