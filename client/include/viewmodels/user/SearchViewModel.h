...
//      • Recent searches + popular searches (per-session cache)
...
class SearchViewModel : public UserViewModelBase {
    ...
    // hasError and error are inherited from UserViewModelBase — no need to
    // re-declare them here. Previously this class shadowed m_error, hasError(),
    // and error() from the base, which caused MOC warnings and broke
    // setError() / clearError() (they wrote to base::m_error while QML
    // read from SearchViewModel::m_error, which was always empty).
...
public:
    ...
    // hasError() and error() are inherited from UserViewModelBase.
...
private:
    ...
    // BUG FIX (Issue 34): separate debounce timer for suggestion refresh
    // so we don't fire a blocking search on every keystroke.
    QTimer m_suggestionDebounceTimer;
